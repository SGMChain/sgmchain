...
namespace sigmaengine { namespace protocol {
   ...
   void vote_transaction_fee_operation::validate()const
   {
      try {
         ...
         FC_ASSERT( trx_fee.symbol == SGT_SYMBOL, "Transaction fee symbol is not available." );
      } FC_CAPTURE_AND_RETHROW( ( *this ) )
   }

   void set_mining_account_operation::validate()const
   {
      try {
   ...
   void mining_reward_process_start_operation::validate()const
   {
      try {
   ...
   void mining_reward_process_stop_operation::validate()const
   {
      try {
   ...
   void transfer_mining_reward_operation::validate() const
   { try {
      validate_account_name( from );
      validate_account_name( to );
      FC_ASSERT( amount.amount > 0, "Cannot transfer a negative amount (aka: stealing)" );
