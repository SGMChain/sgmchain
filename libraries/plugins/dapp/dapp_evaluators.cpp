...
namespace sigmaengine { namespace dapp {
   ...
   void create_dapp_evaluator::do_apply( const create_dapp_operation& op )
   {
      try 
      {
         ...
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee"
               , ( "account", op.owner ) );
         ...
         auto dapp = _db.create< dapp_object > ( [&]( dapp_object& dapp_obj ) {
               ...
               dapp_obj.dapp_state = dapp_state_type::PENDING;
...
// process dapp transaction fee
         if ( dapp_transaction_fee.amount > 0 )
         {
            _db.adjust_balance( from_account, -dapp_transaction_fee );
            _db.adjust_dapp_reward_fund_balance( dapp_transaction_fee );
            _db.push_virtual_operation( dapp_fee_virtual_operation( from_account.name, name_itr->dapp_name, dapp_transaction_fee ) );
         }
      } 
   ...
   void update_dapp_key_evaluator::do_apply( const update_dapp_key_operation& op )
   {
      try 
      {
         ...
         FC_ASSERT( name_itr != name_idx.end(), "\"${name}\" dapp is not exist in Sigma.", ( "name", op.dapp_name ) );
...
//         const auto& from_account = _db.get_account( name_itr->owner );
         const auto& from_account = _db.get_account( op.author );     // reward send token publisher -> from user
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee"
               , ( "account", op.author ) );
         ...
         if ( op.parent_author != SIGMAENGINE_ROOT_POST_PARENT )
         {
            ...
            FC_ASSERT( parent->depth < SIGMAENGINE_MAX_COMMENT_DEPTH, "Comment is nested ${x} posts deep, maximum depth is ${y}."
               , ( "x", parent->depth )( "y", SIGMAENGINE_MAX_COMMENT_DEPTH ) );
         }
         ...
         if ( itr == by_permlink_idx.end() )  // START create comment
         {
            ...
            if ( op.parent_author == SIGMAENGINE_ROOT_POST_PARENT )
               FC_ASSERT( ( now - auth.last_root_post ) > SIGMAENGINE_MIN_ROOT_COMMENT_INTERVAL
                  , "You may only post once every 5 minutes.", ( "now", now )( "last_root_post", auth.last_root_post ) );
            else
               FC_ASSERT( ( now - auth.last_post ) > SIGMAENGINE_MIN_REPLY_INTERVAL
...
#ifndef IS_LOW_MEM
               if ( parent->parent_author != SIGMAENGINE_ROOT_POST_PARENT )
               {
         ...
         if ( dapp_transaction_fee.amount > 0 )
         {
            _db.adjust_balance( from_account, -dapp_transaction_fee );
            _db.adjust_dapp_reward_fund_balance( dapp_transaction_fee );
            _db.push_virtual_operation( dapp_fee_virtual_operation(from_account.name, op.dapp_name, dapp_transaction_fee ) );
         }
      } FC_CAPTURE_AND_RETHROW((op))
   }
   ...
   void comment_vote_dapp_evaluator::do_apply(const comment_vote_dapp_operation& o)
   {
      try {
         ...
         const auto& from_account = _db.get_account( o.voter );     // reward send token publisher -> from user
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee"
               , ( "account", o.voter) );
         ...
         if ( dapp_transaction_fee.amount > 0 )
         {
            _db.adjust_balance( from_account, -dapp_transaction_fee );
            _db.adjust_dapp_reward_fund_balance( dapp_transaction_fee );
            _db.push_virtual_operation( dapp_fee_virtual_operation( from_account.name, o.dapp_name, dapp_transaction_fee ) );
         }
   ...
   void delete_comment_dapp_evaluator::do_apply( const delete_comment_dapp_operation& op )
   {
      try {
         ...
         FC_ASSERT( dapp_name_itr != dapp_name_idx.end(), "${name} dapp is not exist", ( "name", op.dapp_name ) );
         ...
         if( comment.parent_author != SIGMAENGINE_ROOT_POST_PARENT )
         {
...
*/
   ...
   void nsta602_create_evaluator::do_apply( const nsta602_create_operation& o )
   {
      try
      {
         const auto& name_idx = _db.get_index< dapp_index >().indices().get< by_name >();
         auto name_itr = name_idx.find( o.dapp_name );
         FC_ASSERT( name_itr != name_idx.end(), "${name} dapp is not exist", ( "name", o.dapp_name ) );
         FC_ASSERT( name_itr->dapp_state == dapp_state_type::APPROVAL, "The state of ${dapp} dapp is ${state}, not APPROVAL", ( "dapp", o.dapp_name )("state", name_itr->dapp_state) );

         const auto& dapp_user_idx = _db.get_index< dapp_user_index >().indicies().get< by_name >();
         auto user_itr = dapp_user_idx.find( std::make_tuple( o.dapp_name, o.author ) );
         FC_ASSERT( user_itr != dapp_user_idx.end() && user_itr->account_name == o.author, "${author} isn't member of ${dapp} dapp", ( "author", o.author )( "dapp", o.dapp_name ) );
         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         const auto& from_account = _db.get_account( o.author ); 
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee", ( "account", o.author ) );

         const auto& by_permlink_idx = _db.get_index< dapp_nsta602_index >().indices().get< by_permlink >();
         auto itr = by_permlink_idx.find( boost::make_tuple( o.dapp_name, o.author, o.unique_id ) );

         if ( o.json_meta.size() )
         {
            FC_ASSERT( fc::is_utf8(o.json_meta), "JSON Metadata must be UTF-8" );
         }

         FC_ASSERT ( itr == by_permlink_idx.end(), "unique_id already exist." ) ;

         _db.create< dapp_nsta602_object >([&](dapp_nsta602_object& nft)
            {
               nft.author = o.author;
               from_string( nft.unique_id, o.unique_id );
               nft.dapp_name = o.dapp_name;
               nft.init_supply = o.init_supply;
#ifndef IS_LOW_MEM
               if ( o.info.size() < 1024 * 1024 * 128 )
               {
                  from_string( nft.info, o.info );
               }
               if ( fc::is_utf8( o.json_meta ) )
                  from_string( nft.json_meta, o.json_meta );
               else
                  wlog( "NSTA602 ${a}/${p} contains invalid UTF-8 metadata", ( "a", o.author )( "p", o.unique_id ) );
#endif
            });

         _db.create< dapp_nsta602_owner_object >([&](dapp_nsta602_owner_object& ow)
         {
            ow.dapp_name = o.dapp_name;
            ow.author = o.author;
            from_string( ow.unique_id, o.unique_id );
            ow.amount = o.init_supply;
            ow.owner = o.author;
         });
   ...
   void nsta602_transfer_evaluator::do_apply( const nsta602_transfer_operation& o )
   {
      try
      {
         const auto& name_idx = _db.get_index< dapp_index >().indices().get< by_name >();
         ...
         const asset dapp_transaction_fee = _db.get_dynamic_global_properties().dapp_transaction_fee;
         const auto& from_account = _db.get_account( name_itr->owner ); 
         FC_ASSERT( from_account.balance >= dapp_transaction_fee, "Balance of ${account} account is less than dapp transaction fee", ( "account", name_itr->owner ) );

         const auto& by_permlink_idx = _db.get_index< dapp_nsta602_index >().indices().get< by_permlink >();
         auto itr = by_permlink_idx.find( boost::make_tuple( o.dapp_name, o.author, o.unique_id ) );

         dapp_nsta602_owner_id_type id;

         FC_ASSERT ( itr != by_permlink_idx.end(), "nsta602(${dapp} ${author} ${unique_id}) is not exist.", ("dapp", o.dapp_name)("author", o.author)("unique_id", o.unique_id) ) ;

         const auto& nft_owner = _db.get_index<dapp_nsta602_owner_index>().indices().get<by_nsta602_owner>();
         auto from_itr = nft_owner.find(std::make_tuple( o.from, o.dapp_name, o.author, o.unique_id ));
         auto to_itr = nft_owner.find(std::make_tuple( o.to, o.dapp_name, o.author, o.unique_id ));

         FC_ASSERT( from_itr != nft_owner.end(), "${name} is not owner", ( "name", o.from ) );
         FC_ASSERT( from_itr->amount >= o.amount, "Balance of ${account} account doesn't have this nsta602 ", ( "account", o.from ) );

         FC_ASSERT( o.amount != 0 || itr->author == o.from, "${account} account have no authority to transfer.", ( "account", o.from ) );
         FC_ASSERT( o.amount != 0 || to_itr == nft_owner.end(), "${account} account already have this nsta602.", ( "account", o.to ) );

         if ( to_itr != nft_owner.end() )
         {
            // modify
            const auto& owner = *to_itr;

            _db.modify( owner, [&](dapp_nsta602_owner_object& ow )
            {
               ow.amount = ow.amount + o.amount;
            });
         }
         else
         {
            const auto& new_owner = _db.create< dapp_nsta602_owner_object >([&](dapp_nsta602_owner_object& ow)
            {
               ow.dapp_name = o.dapp_name;
               ow.author = o.author;
               from_string( ow.unique_id, o.unique_id );
   ...
   void nsta602_extransfer_evaluator::do_apply( const nsta602_extransfer_operation& o )
   {
      try
      {
         const auto& name_idx = _db.get_index< dapp_index >().indices().get< by_name >();
