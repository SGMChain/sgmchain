...
namespace sigmaengine { namespace dapp {
   ...
   struct dapp_comment_vote_api_object
   {
      ...
      dapp_comment_vote_api_object( const dapp_comment_vote_object& o, const sigmaengine::chain::database& db ) :
         time( o.last_update )
      {
   ...
   struct dapp_account_vote_api_object
   {
      ...
      dapp_account_vote_api_object( const dapp_comment_vote_object o, const sigmaengine::chain::database& db) :
         dapp_name( o.dapp_name )
         , type( o.vote_type )
         , time( o.last_update )
      {
   ...
   struct dapp_nsta602_api_object
   {
      dapp_nsta602_api_object() {}
      dapp_nsta602_api_object( const dapp_nsta602_object& o ) 
         :  id(o.id),
            dapp_name( o.dapp_name ),
            author( o.author ),
            unique_id( to_string(o.unique_id) ),
            init_supply( o.init_supply ),
            info( to_string(o.info) ),
            json_meta( to_string(o.json_meta) ){}

      dapp_nsta602_id_type    id;
      dapp_name_type          dapp_name;
      account_name_type       author;
      string                  unique_id;
   ...
   struct dapp_nsta602_owner_api_object
   {
      dapp_nsta602_owner_api_object() {}
      dapp_nsta602_owner_api_object( const dapp_nsta602_owner_object& o ) 
         :  id(o.id),
            dapp_name( o.dapp_name ),
            author( o.author ),
            unique_id( to_string(o.unique_id) ),
...
FC_REFLECT( sigmaengine::dapp::dapp_api_object, 
   ( dapp_name )
   ( owner )
   ( dapp_key )
   ( dapp_state )
...
FC_REFLECT( sigmaengine::dapp::dapp_comment_vote_api_object, 
   (voter)
   (time) 
)

FC_REFLECT( sigmaengine::dapp::dapp_account_vote_api_object, 
   ( dapp_name )
   ( author )
   ( permlink )
   ( type )
...
FC_REFLECT( sigmaengine::dapp::dapp_comment_api_obj,
   (id)
   (dapp_name)
   (category)
   (parent_author)
...
FC_REFLECT_DERIVED( sigmaengine::dapp::dapp_discussion, (sigmaengine::dapp::dapp_comment_api_obj), 
   ( root_title )
   ( like_votes )
   ( dislike_votes )
   ( replies )
...
FC_REFLECT( sigmaengine::dapp::simple_dapp_discussion, 
   ( id )
   ( dapp_name )
   ( author )
   ( permlink )
...
FC_REFLECT( sigmaengine::dapp::dapp_user_api_object, 
   ( dapp_id )
   ( dapp_name )
   ( account_id )
   ( account_name )
...
FC_REFLECT( sigmaengine::dapp::dapp_vote_api_object, 
   ( dapp_name )
   ( voter )
   ( vote )
   ( last_update )
...
FC_REFLECT( sigmaengine::dapp::dapp_nsta602_api_object, 
    (id)
    (dapp_name)
    (author)
    (unique_id)
...
FC_REFLECT( sigmaengine::dapp::dapp_nsta602_owner_api_object, 
    (id)
    (dapp_name)
    (author)
    (unique_id)
...
FC_API( sigmaengine::dapp::dapp_api,
   ( lookup_dapps )
   ( get_dapp )
   ( get_dapps_by_owner )
   ( get_dapp_content )
