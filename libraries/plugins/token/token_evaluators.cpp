...
#include <sigmaengine/token/token_objects.hpp>
#include <sigmaengine/token/token_plugin.hpp>
#include <sigmaengine/token/util/token_util.hpp>
...
namespace sigmaengine { namespace token {
   ...
   void create_token_evaluator::do_apply( const create_token_operation& op )
   {
      try
      {
         ...
         const auto& dapp_idx = _db.get_index< sigmaengine::dapp::dapp_index >().indices().get< sigmaengine::dapp::by_name >();
         ...
         for( unsigned int i = 0; i < op.symbol_name.length() && i < 6 ; i++ ) 
         {
         ...
         /*
         FC_ASSERT( upper_symbol != from_account.balance.symbol_name()
               , "Symbol can't use ${snac}.", ( "snac", from_account.balance.symbol_name() ) );
         */
   ...
   void issue_token_evaluator::do_apply( const issue_token_operation& op )
   {
      try
      {
         ...
         asset max_token = asset( SIGMAENGINE_TOKEN_MAX * op.reissue_amount.precision(), op.reissue_amount.symbol );
         ...
         if ( dapp_transaction_fee.amount > 0 )
         {
            _db.adjust_balance( from_account, -dapp_transaction_fee );
            _db.adjust_dapp_reward_fund_balance( dapp_transaction_fee );
            _db.push_virtual_operation( dapp_fee_virtual_operation( from_account.name, dapp_itr->dapp_name, dapp_transaction_fee ) );
         }
      }
      FC_CAPTURE_AND_RETHROW( ( op ) )
   }
   ...
   void transfer_token_evaluator::do_apply( const transfer_token_operation& op )
   {
      try {
         ...
         const auto& from_account = _db.get_account( op.from );     // reward send token publisher -> from user
         ...
         if ( dapp_transaction_fee.amount > 0 )
         {
            _db.adjust_balance( from_account, -dapp_transaction_fee );
            _db.adjust_dapp_reward_fund_balance( dapp_transaction_fee );
            _db.push_virtual_operation( dapp_fee_virtual_operation( from_account.name, token_itr->dapp_name, dapp_transaction_fee ) );
         }
      } FC_CAPTURE_AND_RETHROW( ( op ) )
   }
   ...
   void set_token_staking_interest_evaluator::do_apply( const set_token_staking_interest_operation& op )
   {
      try {
         ...
         int64_t precision = SIGMAENGINE_STAKING_INTEREST_PRECISION;

         if( dot_pos != std::string::npos ) {
            auto intpart = op.percent_interest_rate.substr( 0, dot_pos );
            auto temp_frac = op.percent_interest_rate.substr( dot_pos + 1 );

            if( temp_frac.size() - 1 < SIGMAENGINE_STAKING_INTEREST_PRECISION_DIGITS ){
               temp_frac.insert( temp_frac.size(), ( SIGMAENGINE_STAKING_INTEREST_PRECISION_DIGITS - temp_frac.size() ), '0' );
            }
         ...
         if( percent_interest_rate == ( -1 * SIGMAENGINE_STAKING_INTEREST_PRECISION ) ) {
            //remove
...
//         const auto& from_account = _db.get_account( token_itr->publisher );  // token publisher is owner of dapp
         ...
         auto temp_amount = (op.amount.amount.value * ( interest_itr->percent_interest_rate / SIGMAENGINE_STAKING_INTEREST_PRECISION ) ) / 100.0;
...
//         const auto& from_account = _db.get_account( token_itr->publisher );  // token publisher is owner of dapp
         const auto& from_account = _db.get_account( op.from );     // reward send token publisher -> from user
         FC_ASSERT( from_account.balance >= dapp_transaction_fee
