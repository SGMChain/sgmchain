...
namespace sigmaengine { namespace token {
   ...
   void create_token_operation::validate()const
   {
      try {
         ...
         FC_ASSERT( symbol_name.length() > 0  && symbol_name.length() < 6, "Symbol character is less than 6 digits." );

         vector< string > prohibit_names;
         prohibit_names.emplace_back ("sigmaengine");
         ...
         FC_ASSERT( init_supply_amount <= SIGMAENGINE_TOKEN_MAX, "Initial supply amount is over max (90,000,000,000)." );
   ...
   void set_token_staking_interest_operation::validate()const
   {
      try {
         ...
         if(percent_interest_rate.find(".") != std::string::npos ) {
            ...
            FC_ASSERT( lower_decimal >= 0 && lower_decimal <= SIGMAENGINE_STAKING_INTEREST_PRECISION_DIGITS
   ...
   void transfer_token_savings_operation::validate()const
   {
      try {
         ...
         FC_ASSERT( token.size() > 0, "Token name ${n} is invalid.", ("n", token) );
         FC_ASSERT( amount.amount > 0 );
         FC_ASSERT( split_pay_month >= SIGMAENGINE_TRANSFER_SAVINGS_MIN_MONTH 
               && split_pay_month <= SIGMAENGINE_TRANSFER_SAVINGS_MAX_MONTH );
         FC_ASSERT( request_id >= 0 );
         FC_ASSERT( memo.size() < SIGMAENGINE_MAX_MEMO_SIZE, "Memo is too large" );
         FC_ASSERT( fc::is_utf8( memo ), "Memo is not UTF8" );
      } FC_CAPTURE_AND_RETHROW( ( *this ) )
...
DEFINE_OPERATION_TYPE( sigmaengine::token::token_operation )
