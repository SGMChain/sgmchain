...
#include <sigmaengine/chain/compound.hpp>
#include <sigmaengine/chain/custom_operation_interpreter.hpp>
#include <sigmaengine/chain/database.hpp>
#include <sigmaengine/chain/database_exceptions.hpp>
#include <sigmaengine/chain/db_with.hpp>
...
FC_REFLECT( sigmaengine::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( sigmaengine::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( sigmaengine::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
   ...
   try
   {
      ...
      if( chainbase_flags & chainbase::database::read_write )
      {
         ...
         with_write_lock( [&]()
         {
            ...
            // speedhwang for fast start.. validate_invariants();
         });
...
void database::reindex( const fc::path& data_dir, const fc::path& shared_mem_dir, uint64_t shared_file_size )
{
   try
   {
      ...
      open( data_dir, shared_mem_dir, SIGMAENGINE_INIT_SUPPLY, shared_file_size, chainbase::database::read_write );
      _fork_db.reset();    // override effect of _fork_db.start_block() call in open()

      auto start = fc::time_point::now();
      SIGMAENGINE_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
      ...
      with_write_lock( [&]()
      {
         ...
         while( itr.first.block_num() != last_block_num )
         {
            ...
            } FC_CAPTURE_AND_RETHROW( (cur_block_num) )
         }

         std::cerr << "   reindex complete!\n";
...
const account_auth_object& database::get_auth_token( const account_name_type& account, string authtype )const
{ try {
   return get< account_auth_object, by_auth_type >( boost::make_tuple( account, authtype ) );
} FC_CAPTURE_AND_RETHROW( (account)(authtype) ) }

const account_auth_object* database::find_auth_token( const account_name_type& account, string authtype )const
{
   return find< account_auth_object, by_auth_type >( boost::make_tuple( account, authtype ) );
}
...
uint32_t database::bobserver_participation_rate()const
{
   ...
   return uint64_t(SIGMAENGINE_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
   ...
   )
{
   ...
   auto maximum_block_size = get_dynamic_global_properties().maximum_block_size; //SIGMAENGINE_MAX_BLOCK_SIZE;
   ...
   if( bobserver.running_version != SIGMAENGINE_BLOCKCHAIN_VERSION )
      pending_block.extensions.insert( block_header_extensions( SIGMAENGINE_BLOCKCHAIN_VERSION ) );

   const auto& hfp = get_hardfork_property_object();

   if (bobserver.is_bproducer) 
   {
      if( hfp.current_hardfork_version < SIGMAENGINE_BLOCKCHAIN_HARDFORK_VERSION // Binary is newer hardfork than has been applied
      ...
      else if( hfp.current_hardfork_version == SIGMAENGINE_BLOCKCHAIN_HARDFORK_VERSION // Binary does not know of a new hardfork
            && bobserver.hardfork_version_vote > SIGMAENGINE_BLOCKCHAIN_HARDFORK_VERSION ) // Voting for hardfork in the future, that we do not know of...
      {
   ...
   {
      FC_ASSERT( fc::raw::pack_size(pending_block) <= SIGMAENGINE_MAX_BLOCK_SIZE );
   }
...
void database::pop_block()
{
   try
   {
      ...
      SIGMAENGINE_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
void database::notify_pre_apply_operation( operation_notification& note )
{
   ...
   SIGMAENGINE_TRY_NOTIFY( pre_apply_operation, note )
}

void database::notify_post_apply_operation( const operation_notification& note )
{
   SIGMAENGINE_TRY_NOTIFY( post_apply_operation, note )
}
...
{
   SIGMAENGINE_TRY_NOTIFY( applied_block, block )
}

void database::notify_pre_apply_block( const signed_block& block )
{
   SIGMAENGINE_TRY_NOTIFY( pre_apply_block, block )
}

void database::notify_on_pending_transaction( const signed_transaction& tx )
{
   SIGMAENGINE_TRY_NOTIFY( on_pending_transaction, tx )
}

void database::notify_on_pre_apply_transaction( const signed_transaction& tx )
{
   SIGMAENGINE_TRY_NOTIFY( on_pre_apply_transaction, tx )
}

void database::notify_on_applied_transaction( const signed_transaction& tx )
{
   SIGMAENGINE_TRY_NOTIFY( on_applied_transaction, tx )
}

void database::notify_on_apply_hardfork( const uint32_t hardfork ){
   SIGMAENGINE_TRY_NOTIFY( on_apply_hardfork, hardfork )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = SIGMAENGINE_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / SIGMAENGINE_BLOCK_INTERVAL + 1;
}
...
{
   const auto& null_account = get_account( SIGMAENGINE_NULL_ACCOUNT );
   asset total_pia( 0, SGT_SYMBOL );
...
{
   if( head_block_num() >= SIGMAENGINE_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_funds()
{
   ...
   transaction_fee_reward_object fee_reward = get_fee_reward_fund();
   asset fee = fee_reward.fund_balance;
   if(fee.amount > 0) {
      adjust_tx_reward_fund_balance( -fee );
      adjust_balance( bobserver_account, fee );
      tx_reward_virtual_operation virtual_op = tx_reward_virtual_operation( bobserver_account.name, fee );
      push_virtual_operation( virtual_op );
   }
}
   ...
   while( itr != idx.end() ) {
      ...
      if ( itr->split_pay_order == itr->split_pay_month ) {
         ...
         create<savings_withdraw_object>( [&]( savings_withdraw_object& s ) {
            ...
            s.complete        = complete + SIGMAENGINE_TRANSFER_SAVINGS_CYCLE;
         });
      }
   }
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + SIGMAENGINE_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::initialize_evaluators()
{
   ...
   _my->_evaluator_registry.register_evaluator< custom_json_dapp_evaluator                   >();
   ...
   _my->_evaluator_registry.register_evaluator< vote_transaction_fee_evaluator               >(); 
   _my->_evaluator_registry.register_evaluator< set_mining_account_evaluator                 >(); 
   _my->_evaluator_registry.register_evaluator< mining_reward_process_start_evaluator        >(); 
   _my->_evaluator_registry.register_evaluator< mining_reward_process_stop_evaluator         >(); 
   _my->_evaluator_registry.register_evaluator< transfer_mining_reward_evaluator             >(); 
...
void database::initialize_indexes()
{
   ...
   add_core_index< account_auth_index                      >(*this);
   ...
   add_core_index< transaction_fee_vote_index              >(*this);
   add_core_index< transaction_fee_reward_index            >(*this);
   add_core_index< mining_reward_turn_index                >(*this);
...
void database::init_genesis( uint64_t init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(SIGMAENGINE_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = SIGMAENGINE_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SIGMAENGINE_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = SIGMAENGINE_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SIGMAENGINE_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = SIGMAENGINE_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = SIGMAENGINE_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < SIGMAENGINE_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = SIGMAENGINE_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, SGT_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = SIGMAENGINE_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.account      = SIGMAENGINE_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_bobserver = SIGMAENGINE_INIT_MINER_NAME;
         p.time = SIGMAENGINE_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, SGT_SYMBOL );
         p.maximum_block_size = SIGMAENGINE_MAX_BLOCK_SIZE;
         p.dapp_transaction_fee = SIGMAENGINE_DAPP_TRANSACTION_FEE;
         p.transaction_fee = SIGMAENGINE_TRANSACTION_FEE;
      } );
      ...
      {
         cfo.name = SIGMAENGINE_DEPOSIT_FUND_NAME;
         cfo.last_update = head_block_time();
         cfo.fund_balance = asset(0,SGT_SYMBOL);
         cfo.fund_withdraw_ready = asset(0,SGT_SYMBOL);
         for (int i = 0 ; i < SIGMAENGINE_MAX_USER_TYPE ; i++) {
            for (int j = 0 ; j < SIGMAENGINE_MAX_STAKING_MONTH ; j++) {
      ...
      auto fee_reward_fund = create< transaction_fee_reward_object>( [&]( transaction_fee_reward_object& o ) {
               o.fund_balance = asset( 0, SGT_SYMBOL );
               o.reward_block = head_block_num();
            });
      FC_ASSERT( fee_reward_fund.id._id == 0 );
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      SIGMAENGINE_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         SIGMAENGINE_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
uint32_t database::get_free_memory_gb()
{
   uint32_t free_gb = uint32_t( get_free_memory() / (1024*1024*1024) );
...
{ try {
   ...
   if( block_size < SIGMAENGINE_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",SIGMAENGINE_MIN_BLOCK_SIZE)
      );
   }
...
void database::process_transaction_fee()
{
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();

   if ( dpo.head_block_number == dpo.next_refresh_transaction_fee_block )
   {
      const auto& vote_idx = get_index< transaction_fee_vote_index >().indicies().get< by_voter >();
      auto vote_itr = vote_idx.begin();
      vector< asset > trx_fee_list;
      while( vote_itr != vote_idx.end() ) {
         if( dpo.head_block_number < vote_itr->vote_block + dpo.refresh_transaction_fee_cycle ){
            trx_fee_list.push_back( vote_itr->trx_fee );
         }
         vote_itr++;
      }

      if( trx_fee_list.size() >= SIGMAENGINE_MIN_FEEDS ) {
         std::sort( trx_fee_list.begin(), trx_fee_list.end() );
         auto median_value = trx_fee_list[ trx_fee_list.size()/2 ];

         modify( dpo, [&]( dynamic_global_property_object& object ) {
            object.transaction_fee = median_value;
            object.prev_refresh_transaction_fee_block = dpo.head_block_number;
            object.next_refresh_transaction_fee_block = dpo.head_block_number + dpo.refresh_transaction_fee_cycle;
         });

         ilog( "process_transaction_fee : median_value = ${med}", ( "med", median_value ) );
      }
      else
      {
         modify( dpo, [&]( dynamic_global_property_object& object ) {
...
{ try {
   ...
   const chain_id_type& chain_id = SIGMAENGINE_CHAIN_ID;
   auto trx_id = trx.id();
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, SIGMAENGINE_MAX_SIG_CHECK_DEPTH );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         SIGMAENGINE_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      SIGMAENGINE_ASSERT( trx.expiration <= now + fc::seconds(SIGMAENGINE_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",SIGMAENGINE_MAX_TIME_UNTIL_EXPIRATION));
      SIGMAENGINE_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      SIGMAENGINE_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
...
{ try {
   ...
   {
      SIGMAENGINE_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < SIGMAENGINE_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",SIGMAENGINE_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < SIGMAENGINE_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > SIGMAENGINE_NUM_BOBSERVERS )
            _dpo.last_irreversible_block_num = head_block_num() - SIGMAENGINE_NUM_BOBSERVERS;
      } );
   }
   else
   {
      ...
      static_assert( SIGMAENGINE_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((SIGMAENGINE_100_PERCENT - SIGMAENGINE_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / SIGMAENGINE_100_PERCENT);
...
void database::adjust_balance( const account_object& a, const asset& delta )
{
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol )
      {
         case SGT_SYMBOL:
            acnt.balance += delta;
            if ( dpo.current_reward_turn > 0 && a.mining == 1 )
            {
               acnt.after_mining_balance += delta;
            }
            break;
         default:
...
void database::adjust_tx_reward_fund_balance ( const asset& delta )
{
   modify( get(transaction_fee_reward_id_type()), [&]( transaction_fee_reward_object &object )
   {
      switch( delta.symbol )
      {
         case SGT_SYMBOL:
            object.fund_balance += delta;
            object.reward_block = head_block_num();
            break;
         default:
            FC_ASSERT( !"invalid symbol" );
      }
...
void database::adjust_tx_fee(account_name_type from, string op_name)
{
   const asset transaction_fee = get_dynamic_global_properties().transaction_fee;
   const auto& from_account = get_account( from );
   
   adjust_balance( from_account, -transaction_fee );
   adjust_tx_reward_fund_balance( transaction_fee );
   push_virtual_operation( tx_fee_virtual_operation( from_account.name, op_name, transaction_fee ) );
}
...
{
   _hardfork_times[ 0 ] = fc::time_point_sec( SIGMAENGINE_GENESIS_TIME );
   _hardfork_versions[ 0 ] = hardfork_version( 0, 0 );
   
   FC_ASSERT( SIGMAENGINE_HARDFORK_0_1 == 1, "Invalid hardfork configuration : 0.1" );
   _hardfork_times[ SIGMAENGINE_HARDFORK_0_1 ] = fc::time_point_sec( SIGMAENGINE_HARDFORK_0_1_TIME );
   _hardfork_versions[ SIGMAENGINE_HARDFORK_0_1 ] = SIGMAENGINE_HARDFORK_0_1_VERSION;
   
   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= SIGMAENGINE_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("SIGMAENGINE_NUM_HARDFORKS",SIGMAENGINE_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions[ hardforks.last_hardfork ] <= SIGMAENGINE_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( SIGMAENGINE_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions[ SIGMAENGINE_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         while( hardforks.last_hardfork < SIGMAENGINE_NUM_HARDFORKS
               && _hardfork_times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < SIGMAENGINE_HARDFORK_0_1 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= SIGMAENGINE_NUM_HARDFORKS; i++ )
   {
      /*if( i <= SIGMAENGINE_HARDFORK_0_5__54 )
...
std::map<asset_symbol_type, asset> database::get_total_supply() const
{
   ...
   asset total_supply = asset(0, SGT_SYMBOL);
...
void database::validate_invariants()const
{
   ...
   asset total_supply = supply_list.find( SGT_SYMBOL )->second;
...
const transaction_fee_reward_object& database::get_fee_reward_fund()const
{
   return get(transaction_fee_reward_id_type());
}
