...
namespace sigmaengine { namespace chain {
   ...
   class database : public chainbase::database
   {
      public:
         ...
         void process_transaction_fee();

         const common_fund_object&              get_common_fund( const string name )const;
         const dapp_reward_fund_object&         get_dapp_reward_fund()const;
         const transaction_fee_reward_object&   get_fee_reward_fund()const;
         void adjust_dapp_reward_fund_balance( const asset& delta );
         void adjust_tx_reward_fund_balance( const asset& delta );
         void adjust_tx_fee(account_name_type from, string op_name);

         uint32_t get_free_memory_gb();
