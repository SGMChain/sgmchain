...
void account_create_evaluator::do_apply( const account_create_operation& o )
{
   ...
   const auto& from_account = _db.get_account( o.creator );
   const asset transaction_fee = props.transaction_fee;
   FC_ASSERT( from_account.balance >= transaction_fee, "Balance of ${account} account is less than transaction fee", ( "account", o.creator ) );
   ...
   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(o.creator, "account_create_operation");
   }
}
...
void account_update_evaluator::do_apply( const account_update_operation& o )
{
   ...
   const asset transaction_fee = _db.get_dynamic_global_properties().transaction_fee;
   FC_ASSERT( account.balance >= transaction_fee, "Balance of ${account} account is less than transaction fee", ( "account", o.account ) );
   ...
   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(o.account, "account_create_operation");
   }
...
void transfer_evaluator::do_apply( const transfer_operation& o )
{
   ...
   FC_ASSERT( _db.get_balance( from_account, amount.symbol ) >= amount + transaction_fee, "Account does not have sufficient funds for transfer." );
   ...
   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(o.from, "transfer_operation");
   }
}
...
void transfer_savings_evaluator::do_apply( const transfer_savings_operation& op )
{
   ...
   FC_ASSERT( _db.get_balance( from, amount.symbol ) >= amount + transaction_fee, "Account does not have sufficient funds for transfer." );
   ...
   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(op.from, "transfer_savings_operation");
   }
}
...
void cancel_transfer_savings_evaluator::do_apply( const cancel_transfer_savings_operation& op )
{
   ...
   const asset transaction_fee = _db.get_dynamic_global_properties().transaction_fee;
   FC_ASSERT( from_account.balance >= transaction_fee, "Balance of ${account} account is less than transaction fee", ( "account", op.from ) );
   ...
   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(op.from, "cancel_transfer_savings_operation");
   }
}
...
void conclusion_transfer_savings_evaluator::do_apply( const conclusion_transfer_savings_operation& op )
{
   ...
   _db.push_virtual_operation( fill_transfer_savings_operation( swo.from, swo.to, swo.amount, swo.total_amount, swo.split_pay_month, swo.split_pay_month, swo.request_id, to_string( swo.memo) ) );
   _db.remove( swo );

   if ( transaction_fee.amount > 0 )
   {
      _db.adjust_tx_fee(op.from, "conclusion_transfer_savings_operation");
   }
...
void vote_transaction_fee_evaluator::do_apply(const vote_transaction_fee_operation& o)
{
   try {
         dlog( "vote_transaction_fee_evaluator::do_apply" );

         const auto& bo_idx = _db.get_index< bobserver_index >().indicies().get< chain::by_bp_owner >();
         auto bo_itr = bo_idx.find( o.voter );
         FC_ASSERT( bo_itr != bo_idx.end() && bo_itr->bp_owner == o.voter && bo_itr->is_bproducer, "${account} is not bp", ( "account", o.voter ) );

         _db.modify( *bo_itr, [&]( bobserver_object& bo ) {
            bo.tx_fee_vote = o.trx_fee;
         });

         const auto& vote_idx = _db.get_index< transaction_fee_vote_index >().indicies().get< by_voter >();
         auto vote_itr = vote_idx.find( o.voter );
         if( vote_itr == vote_idx.end() ){
            _db.create< transaction_fee_vote_object >( [&]( transaction_fee_vote_object& object ) {
               object.voter = o.voter;
               object.trx_fee = o.trx_fee;
               object.vote_block = _db.head_block_num();
            });
         } else {
            _db.modify( *vote_itr, [&]( transaction_fee_vote_object& object ) {
               object.trx_fee = o.trx_fee;
               object.vote_block = _db.head_block_num();
            });
         }
...
void set_mining_account_evaluator::do_apply(const set_mining_account_operation& o)
{
   try {
         const auto& origin = _db.get_account(SIGMAENGINE_ROOT_ACCOUNT);
         const auto& bypass = _db.get_account( o.root );
         ...
         FC_ASSERT (!( (o.mining && account.mining == 1) || (!o.mining && account.mining == 0) ));
         
         _db.modify( account, [&]( account_object& object ) {
            object.mining = o.mining ? 1 : 0;
         });
...
void mining_reward_process_start_evaluator::do_apply(const mining_reward_process_start_operation& o)
{
   try {
         
         dlog( "mining_reward_process_start_evaluator::do_apply" );

         const dynamic_global_property_object& _dgp = _db.get_dynamic_global_properties();
         _db.modify( _dgp, [&]( dynamic_global_property_object& dgp )
         {
            dgp.current_reward_turn = o.turn;
         });

         _db.create< mining_reward_turn_object >( [&]( mining_reward_turn_object& object ) {
            object.turn = o.turn;
            object.amount = o.amount;
            object.start_block = _db.head_block_num();
            object.stop_block = 0;
         });
...
void mining_reward_process_stop_evaluator::do_apply(const mining_reward_process_stop_operation& o)
{
   try {

         dlog( "mining_reward_process_stop_evaluator::do_apply" );

         const auto& idx = _db.get_index< mining_reward_turn_index >().indicies().get< by_turn >();
         auto itr = idx.find( o.turn );
         
         FC_ASSERT( itr != idx.end(), "There are no active reward process." );

         _db.modify( *itr, [&]( mining_reward_turn_object& object ) {
            object.stop_block = _db.head_block_num();
         });

         const dynamic_global_property_object& _dgp = _db.get_dynamic_global_properties();
         _db.modify( _dgp, [&]( dynamic_global_property_object& dgp )
         {
            dgp.current_reward_turn = 0;
         });

         const auto& mining_idx = _db.get_index< account_index >().indicies().get< by_mining >();
         auto m_itr = mining_idx.lower_bound(1);
         auto m_end = mining_idx.upper_bound(1);

         while(m_itr != m_end)
         {
            _db.modify( *m_itr, [&]( account_object& object ) {
               object.after_mining_balance = asset( 0, SGT_SYMBOL );
            });
...
void transfer_mining_reward_evaluator::do_apply( const transfer_mining_reward_operation& o )
{
   const auto& origin = _db.get_account(SIGMAENGINE_REWARD_ACCOUNT);
   
   const auto& from_account = _db.get_account(o.from);
   const auto& to_account = _db.get_account(o.to);

   FC_ASSERT(origin.name     == from_account.name);
