...
#include <sigmaengine/token/token_api.hpp>
...
using namespace sigmaengine::dapp_history;
...
class wallet_api
{
   public:
      ...
      /** Erase a WIF Private Key into the wallet to be used to sign transactions by an account.
       *
       * example: erase_key 5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3
       *
       * @param wif_key the WIF Private Key to erase
       */
       ...
       * @returns list of BP
       * */
      vector< bproducer_api_obj > list_bproducers();
       ...
       *                       operations defined in `sigmaengine/chain/operations.hpp`
...
//      uint64_t get_account_transfer_history_count( string account, uint32_t type );
      ...
      map<uint32_t,applied_operation> get_account_token_symbol_transfer_history( string account, string symbol, uint32_t from, uint32_t limit );
      map<uint32_t,applied_operation> get_account_token_transfer_history( string account, uint32_t from, uint32_t limit );
      ...
      map< uint32_t, uint64_t > get_transaction_day_count(uint32_t day)const;
       ...
       * @param authtype user auth type
       * @param authtoken user auth token value
       * */
      annotated_signed_transaction set_auth_token( string account, string authtype, string authtoken,
                                          bool broadcast = false);           
      ...
      annotated_signed_transaction nsta602_transfer( string from, string to, string dapp_name, string author, string unique_id, uint64_t amount, bool broadcast);
      annotated_signed_transaction nsta602_extransfer( string from, string to, string dapp_name, string author, string unique_id, string price, string tx_id, string memo, uint64_t amount, bool broadcast);

      /**
       * create nsta602 a dapp
       * @param dapp_name dapp name
       * @param author the name of the account authoring the nsta602
       * @param unique_id the accountwide unique id for the nsta602
       * @param init_supply
       * @param info
       * @param json_meta the json metadata of the nsta602
       * @param broadcast true if you wish to broadcast the transaction
       * @return annotated_signed_transaction
       * */
      annotated_signed_transaction nsta602_create( string dapp_name, string author, string unique_id, uint64_t init_supply, string info, string json_meta, bool broadcast );

      optional <dapp_nsta602_api_object> get_nsta602( string dapp_name, string author, string unique_id );
      vector <dapp_nsta602_api_object> get_nsta602_by_dapp( string dapp_name, uint32_t from, uint32_t limit, bool newest );
      vector <dapp_nsta602_owner_api_object> get_nsta602_owners_by_amount( string dapp_name, string author, string unique_id );
      vector <dapp_nsta602_owner_api_object> get_nsta602_owners( string dapp_name, string author, string unique_id, uint32_t from, uint32_t limit, bool newest );
      optional <dapp_nsta602_owner_api_object> search_my_nsta602( string dapp_name, string author, string unique_id, string owner );
      vector <dapp_nsta602_owner_api_object> get_nsta602_by_owner( string owner, uint32_t from, uint32_t limit, bool newest );
      vector <dapp_nsta602_api_object> get_nsta602_by_author( string author, uint32_t from, uint32_t limit, bool newest );
      ...
      /**
       * Vote for transaction fee of DApp. Only BP can vote.
      ...
      annotated_signed_transaction vote_transaction_fee ( string voter, asset fee, bool broadcast );
      ...
      map< uint32_t, applied_operation > get_nsta602_transfer_history( string dapp_name, string author, string unique_id, uint64_t from, uint32_t limit );


      string get_auth_token( string account, string authtype );
      vector<account_auth_api_obj> get_auth_token_list( uint64_t from, uint32_t limit );
       ...
       */
      annotated_signed_transaction cancel_transfer_savings( string from, string to, asset amount, uint32_t request_id, bool broadcast = false );

      /**
       *  @param request_id the id used in transfer_savings
       *  @param from the account that initiated the transfer
       */
      annotated_signed_transaction conclusion_transfer_savings( string from, string to, uint32_t request_id, bool broadcast = false );
      ...
      annotated_signed_transaction set_mining_account(string account, bool mining, bool broadcast);
      annotated_signed_transaction start_mining_reward_process(uint32_t turn, asset amount, bool broadcast);
      annotated_signed_transaction stop_mining_reward_process(uint32_t turn, bool broadcast);
      annotated_signed_transaction transfer_mining_reward(string from, string to, asset amount, string memo, bool broadcast );

      vector<mining_reward_turn_api_obj> get_reward_turn_list( uint32_t from, uint32_t limit );
      map< uint32_t, account_mining_balance_api_obj > get_mining_accounts( uint32_t from, uint32_t limit ) ;

      map<uint32_t,applied_operation> get_dapp_operation_list( uint32_t from, uint32_t limit ) ;
...
} // namespace sigmaengine::wallet 
} // namespace sigmaengine

FC_REFLECT( sigmaengine::wallet::wallet_data,
            (cipher_keys)
            (ws_server)
            (ws_user)
            (ws_password)
...
FC_REFLECT( sigmaengine::wallet::brain_key_info, (brain_priv_key)(wif_priv_key) (pub_key))

FC_REFLECT( sigmaengine::wallet::plain_keys, (checksum)(keys) )

FC_REFLECT( sigmaengine::wallet::creating_dapp_result, (trx)(dapp_key) )

FC_REFLECT_ENUM( sigmaengine::wallet::authority_type, (owner)(active)(posting))

FC_API( sigmaengine::wallet::wallet_api,
        /// wallet api
        (help)(gethelp)
        ...
        (get_account_token_symbol_transfer_history )
        ( get_account_token_transfer_history )
        ( get_account_transfer_history )
        ( get_transaction_count )
        ( get_block_range )
        ( get_dapp_operation_list )
        ...
        ( get_nsta602 )
        ( get_nsta602_by_dapp )
        ( get_nsta602_owners_by_amount )
        ( get_nsta602_owners )
        ( search_my_nsta602 )
        ( get_nsta602_by_owner )
        ( get_nsta602_by_author )

        ( get_free_memory )

        ( set_mining_account )
        ( start_mining_reward_process )
        ( stop_mining_reward_process )

        ( get_reward_turn_list )
        ( get_mining_accounts )
