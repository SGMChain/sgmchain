...
#include <sigmaengine/wallet/wallet.hpp>
#include <sigmaengine/wallet/api_documentation.hpp>
#include <sigmaengine/wallet/reflect_util.hpp>

#include <sigmaengine/dapp/dapp_plugin.hpp>
#include <sigmaengine/token/token_operations.hpp>

#include <sigmaengine/account_by_key/account_by_key_api.hpp>
...
namespace sigmaengine { namespace wallet {
...
public:
   ...
   variant_object about() const
   {
      ...
      result["blockchain_version"]       = SIGMAENGINE_BLOCKCHAIN_VERSION;
      result["sw_version"]               = SIGMAENGINE_VERSION;
   ...
   bool import_key(string wif_key)
   {
      ...
      sigmaengine::chain::public_key_type wif_pub_key = optional_private_key->get_public_key();

      _keys[wif_pub_key] = wif_key;
      return true;
   }

   bool erase_key(string wif_key)
   {
      fc::optional<fc::ecc::private_key> optional_private_key = wif_to_key(wif_key);
      if (!optional_private_key)
         FC_THROW("Invalid private key");
   ...
   {
      _wallet.ws_server = wdata.ws_server;
      _wallet.ws_user = wdata.ws_user;
      _wallet.ws_password = wdata.ws_password;
   }
   ...
   int find_first_unused_derived_key_index(const fc::ecc::private_key& parent_key)
   {
      ...
      for (int key_index = 0; ; ++key_index)
      {
         ...
         sigmaengine::chain::public_key_type derived_public_key = derived_private_key.get_public_key();
   ...
   {
      FC_ASSERT( tx_expiration_seconds < SIGMAENGINE_MAX_TIME_UNTIL_EXPIRATION );
...
*/
   ...
   {
      for( const public_key_type& k : tx.pkeys )
      {
         auto it = _keys.find(k);
         FC_ASSERT( it != _keys.end() );
         fc::optional<fc::ecc::private_key> privkey = wif_to_key( it->second );
         FC_ASSERT( privkey.valid(), "Malformed private key in _keys" );
         tx.sign( *privkey, SIGMAENGINE_CHAIN_ID );
      }

      return tx;
   }

   annotated_signed_transaction fill_transaction( signed_transaction tx, bool broadcast = false )
   {
      if( broadcast ) {
         try {
   ...
   annotated_signed_transaction sign_transaction(signed_transaction tx, bool broadcast = false)
   {
      ...
      for( const public_key_type& k : minimal_signing_keys )
      {
         ...
         tx.sign( it->second, SIGMAENGINE_CHAIN_ID );
      }
   ...
   std::map<string,std::function<string(fc::variant,const fc::variants&)>> get_result_formatters() const
   {
      ...
      m["list_my_accounts"] = [](variant result, const fc::variants& a ) {
         ...
         asset total_pia = asset( 0, SGT_SYMBOL );
...
bool wallet_api::erase_key(string wif_key)
{
   FC_ASSERT(!is_locked());
...
annotated_signed_transaction wallet_api::burn(string account, asset amount, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    burn_operation op;
...
annotated_signed_transaction wallet_api::create_token( string dapp_name, string creator, string token_name, string symbol, int64_t init_supply, bool broadcast )
{
   try {
      ...
      custom_json_dapp_operation custom_op;
...
annotated_signed_transaction wallet_api::return_staking_fund(string fund_name, uint32_t request_id, string to, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
    FC_ASSERT( !is_locked() );
   
    return_staking_fund_operation op;
    op.root = SIGMAENGINE_ROOT_ACCOUNT;
    op.fund_name = fund_name;
    op.request_id = request_id;
    op.to = to;
   ...
   return my->sign_transaction( tx, broadcast );
} FC_CAPTURE_AND_RETHROW( (fund_name)(request_id)(to)(broadcast) ) }
...
annotated_signed_transaction wallet_api::cancel_transfer_savings( string from, string to, asset amount, uint32_t request_id, bool broadcast  )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
...
annotated_signed_transaction wallet_api::conclusion_transfer_savings( string from, string to, uint32_t request_id, bool broadcast  )
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
...
creating_dapp_result wallet_api::create_dapp ( string owner, string name, bool broadcast )
{
   try
   {
      ...
      custom_json_dapp_operation custom_operation;
...
annotated_signed_transaction wallet_api::nsta602_transfer( string from, string to, string dapp_name, string author, string unique_id, uint64_t amount, bool broadcast)
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
      
      nsta602_transfer_operation op;
      op.from = from;
      op.to = to;
      op.dapp_name = dapp_name;
      op.author = author;
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( (from) (to) (dapp_name) (author) (unique_id) (amount) (broadcast) )
}

annotated_signed_transaction wallet_api::nsta602_extransfer( string from, string to, string dapp_name, string author, string unique_id, string price, string tx_id, string memo, uint64_t amount, bool broadcast)
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
      
      nsta602_extransfer_operation op;
      op.from = from;
      op.to = to;
      op.dapp_name = dapp_name;
      op.author = author;
...
annotated_signed_transaction wallet_api::nsta602_create( string dapp_name, string author, string unique_id, uint64_t init_supply, string info, string json_meta, bool broadcast )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );
      
      nsta602_create_operation op;
      op.dapp_name = dapp_name;
      op.author = author;
      op.unique_id = unique_id;
      op.init_supply = init_supply;
      op.info = info;
      ...
      custom_op.id = DAPP_PLUGIN_NAME;
      custom_op.json = fc::json::to_string( plugin_op );
      custom_op.required_active_auths.insert( author );
      ...
      return my->sign_transaction( tx, broadcast );
   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id) (init_supply) (info) (json_meta) (broadcast) )
}

optional <dapp_nsta602_api_object> wallet_api::get_nsta602( string dapp_name, string author, string unique_id )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602( dapp_name, author, unique_id );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id) )
}

vector <dapp_nsta602_api_object> wallet_api::get_nsta602_by_dapp( string dapp_name, uint32_t from, uint32_t limit, bool newest ) {

   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_by_dapp( dapp_name, from, limit, newest );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (from) (limit)(newest) )
}

vector <dapp_nsta602_owner_api_object> wallet_api::get_nsta602_owners_by_amount( string dapp_name, string author, string unique_id )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_owners_by_amount( dapp_name, author, unique_id );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id) )
}

vector <dapp_nsta602_owner_api_object> wallet_api::get_nsta602_owners( string dapp_name, string author, string unique_id, uint32_t from, uint32_t limit, bool newest )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_owners( dapp_name, author, unique_id, from, limit, newest );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id) (from) (limit)(newest) )
}

optional <dapp_nsta602_owner_api_object> wallet_api::search_my_nsta602( string dapp_name, string author, string unique_id, string owner )
{
   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->search_my_nsta602( dapp_name, author, unique_id, owner );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (author) (unique_id)(owner) )
}

vector <dapp_nsta602_owner_api_object> wallet_api::get_nsta602_by_owner( string owner, uint32_t from, uint32_t limit, bool newest ) {

   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_by_owner( owner, from, limit, newest );
      return results;

   } FC_CAPTURE_AND_RETHROW( (owner) (from) (limit)(newest) )
}

vector <dapp_nsta602_api_object> wallet_api::get_nsta602_by_author( string author, uint32_t from, uint32_t limit, bool newest ) {

   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_by_author( author, from, limit, newest );
      return results;

   } FC_CAPTURE_AND_RETHROW( (author) (from) (limit) (newest) )
}

/*
vector <dapp_nsta602_api_object> wallet_api::get_nsta602_owners( string dapp_name, string author, string unique_id, uint32_t from, uint32_t limit ) {

   try {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      
      my->use_dapp_api();
      auto results = (*my->_remote_dapp_api)->get_nsta602_owner( dapp_name, from, limit );
      return results;

   } FC_CAPTURE_AND_RETHROW( (dapp_name) (from) (limit) )
}
*/
...
annotated_signed_transaction wallet_api::vote_transaction_fee ( string voter, asset fee, bool broadcast ){
   try
   {
      FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
      FC_ASSERT( !is_locked() );

      vote_transaction_fee_operation operation;
      operation.voter = voter;
...
map< uint32_t, applied_operation > wallet_api::get_dapp_history( string dapp_name, uint64_t from, uint32_t limit ) {
   ...
   for( auto& item : result ) {
      if( item.second.op.which() == operation::tag< custom_json_dapp_operation >::value ) {
         auto& custom_op = item.second.op.get< custom_json_dapp_operation >();
         try{
...
map< uint32_t, applied_operation > wallet_api::get_nsta602_transfer_history( string dapp_name, string author, string unique_id, uint64_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   my->use_dapp_history_api();
   auto result = (*my->_remote_dapp_history_api)->get_nsta602_transfer_history(dapp_name, author, unique_id, from, limit);
...
vector< bproducer_api_obj > wallet_api::list_bproducers()
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   return my->_remote_db->lookup_bproducer_accounts();
}
...
signed_transaction wallet_api::offline_signature(offline_transaction tx)
{ try {

   FC_ASSERT( !is_locked() );
   
   return my->offline_signature(tx);
} FC_CAPTURE_AND_RETHROW( (tx) ) }

offline_transaction wallet_api::get_sign_publickey(offline_transaction tx)
{ try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   
   return my->get_sign_publickey( tx);
} FC_CAPTURE_AND_RETHROW( (tx) ) }

annotated_signed_transaction wallet_api::fill_transaction( signed_transaction tx, bool broadcast )
{ try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   
   return my->fill_transaction(tx, broadcast);
} FC_CAPTURE_AND_RETHROW( (tx) ) }
...
void wallet_api::set_wallet_info( wallet_data wdata )
{
   my->set_wallet_info(wdata);
}
...
annotated_signed_transaction wallet_api::push_transaction(signed_transaction tx, bool broadcast )
{ try {
   FC_ASSERT( !is_locked() );
...
offline_transaction wallet_api::transfer_offline(string from, string to, asset amount, string memo)
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    check_memo( memo, get_account( from ) );
...
signed_transaction wallet_api::sign_offline_transaction(offline_transaction tx )
{ try {
   FC_ASSERT( !is_locked() );
...
map<uint32_t,applied_operation> wallet_api::get_dapp_operation_list( uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");

   my->use_dapp_history_api();
   auto result = (*my->_remote_dapp_history_api)->get_dapp_operation_list(from, limit);

   return result;
}

map<uint32_t,account_balance_api_obj> wallet_api::get_balance_rank( uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
...
/*
uint64_t wallet_api::get_account_transfer_history_count( string account, uint32_t type ) 
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   
   auto result = my->_remote_db->get_account_transfer_history_count(account,type);

   return result;
}
*/

map<uint32_t,applied_operation> wallet_api::get_account_token_symbol_transfer_history( string account, string symbol, uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   
   auto result = my->_remote_db->get_account_token_symbol_transfer_history(account, symbol, from,limit);

   return result;
}

map<uint32_t,applied_operation> wallet_api::get_account_token_transfer_history( string account, uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   
   auto result = my->_remote_db->get_account_token_transfer_history(account, from,limit);
...
map< uint32_t, uint64_t > wallet_api::get_transaction_day_count(uint32_t day)const
{
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   
   return my->_remote_db->get_transaction_day_count( day );
}

uint32_t wallet_api::get_free_memory() const
{
...
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   account_auth_operation op;

   op.account    = account;
   op.auth_type  = authtype;
   op.auth_token = authtoken;
   ...
   return my->sign_transaction( tx, broadcast );
} FC_CAPTURE_AND_RETHROW( (account)(authtype)(authtoken) ) }

string wallet_api::get_auth_token( string account, string authtype ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   return my->_remote_db->get_auth_token(account,authtype);
}

vector<account_auth_api_obj> wallet_api::get_auth_token_list( uint64_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   return my->_remote_db->get_auth_token_list(from,limit);
}

vector<mining_reward_turn_api_obj> wallet_api::get_reward_turn_list( uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   return my->_remote_db->get_reward_turn_list(from,limit);
}

map< uint32_t, account_mining_balance_api_obj > wallet_api::get_mining_accounts(uint32_t from, uint32_t limit ) {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   return my->_remote_db->get_mining_accounts(from, limit);
}

annotated_signed_transaction wallet_api::set_mining_account(string account, bool mining, bool broadcast)
{try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   set_mining_account_operation op;
...
}FC_CAPTURE_AND_RETHROW( (account)(mining) ) }

annotated_signed_transaction wallet_api::start_mining_reward_process(uint32_t turn, asset amount, bool broadcast)
{try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   mining_reward_process_start_operation op;
...
}FC_CAPTURE_AND_RETHROW( (turn)(amount) ) }

annotated_signed_transaction wallet_api::stop_mining_reward_process(uint32_t turn, bool broadcast)
{try {

   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
   mining_reward_process_stop_operation op;
...
}FC_CAPTURE_AND_RETHROW( (turn) ) }


annotated_signed_transaction wallet_api::transfer_mining_reward(string from, string to, asset amount, string memo, bool broadcast )
{ try {
   FC_ASSERT(my->_wallet.ws_server != "local", "Wallet  is local mode.");
   FC_ASSERT( !is_locked() );
    check_memo( memo, get_account( from ) );
    transfer_mining_reward_operation op;
    op.from = from;
    op.to = to;
    op.amount = amount;
