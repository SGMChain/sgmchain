...
struct account_api_obj
{
   account_api_obj( const chain::account_object& a, const chain::database& db ) :
      ...
      mining ( a.mining ),
      after_mining_balance ( a.after_mining_balance ),
      nsta602_count(a.nsta602_count),
      nsta602_create_count(a.nsta602_create_count),
...
struct account_mining_balance_api_obj {
   account_mining_balance_api_obj(){}

   account_mining_balance_api_obj( account_name_type _name, asset _balance, asset _a_balance) :
      account( _name ), balance( _balance ), a_balance(_a_balance){}
...
struct mining_reward_turn_api_obj
{
   mining_reward_turn_api_obj( const chain::mining_reward_turn_object& o) :
      id(o.id),
      turn(o.turn),
      amount(o.amount),
      start_block(o.start_block),
      stop_block(o.stop_block)
   {}

   mining_reward_turn_api_obj(){}

   mining_reward_turn_id_type       id;
   uint32_t                         turn;
   asset                            amount;
...
FC_REFLECT( sigmaengine::app::mining_reward_turn_api_obj,
             (id)
             (turn)
             (amount)
             (start_block)
...
FC_REFLECT( sigmaengine::app::account_mining_balance_api_obj,
             (account)
             (balance)
             (a_balance)
          )
