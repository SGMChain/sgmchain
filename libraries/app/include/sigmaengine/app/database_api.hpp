...
//      uint64_t get_account_transfer_history_count( string account,uint32_t type )const;
      ...
      map< uint32_t, applied_operation > get_account_token_symbol_transfer_history( string account, string symbol, uint64_t from, uint32_t limit )const;
      map< uint32_t, applied_operation > get_account_token_transfer_history( string account, uint64_t from, uint32_t limit )const;
      ...
      vector<mining_reward_turn_api_obj> get_reward_turn_list(uint32_t from, uint32_t limit) const;
      map< uint32_t, account_mining_balance_api_obj > get_mining_accounts(uint32_t from, uint32_t limit)const;
